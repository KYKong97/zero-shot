...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, pretrained_window_size=0,is_moe=False, num_local_experts=1, top_value=1, 
                 capacity_factor=1.25,cosine_router=False,
                 normalize_gate=False, use_bpr=True, is_gshard_loss=True, gate_noise=1.0,
                 cosine_router_dim=256, cosine_router_init_t=0.5, moe_drop=0.0,mlp_fc2_bias=False,init_std=0.0):
        super().__init__()
        self.dim = dim
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 pretrained_window_size=0,mlp_fc2_bias=True,init_std=0.02,moe_block=[-1], num_local_experts=1, top_value=1, capacity_factor=1.25, cosine_router=False,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 pretrained_window_size=pretrained_window_size,mlp_fc2_bias=mlp_fc2_bias,
                                 init_std=init_std,is_moe=True if i in moe_block else False,
                                 num_local_experts=num_local_experts,
                                 top_value=top_value,
                                 capacity_factor=capacity_factor,
                                 cosine_router=cosine_router,
...
class SwinTransformerV2_MOE(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0], frozen_stages=-1,moe_blocks=[[-1], [-1], [-1], [-1]], num_local_experts=1, top_value=1, 
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               pretrained_window_size=pretrained_window_sizes[i_layer],moe_block=moe_blocks[i_layer],
                               num_local_experts=num_local_experts,
                               top_value=top_value,
                               capacity_factor=capacity_factor,
                               cosine_router=cosine_router,
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformerV2_MOE, self).train(mode)
        self._freeze_stages()
...
class D2SwinTransformerV2_MOE(SwinTransformerV2_MOE, Backbone):
